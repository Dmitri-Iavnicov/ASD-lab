...
int main()
{
    ...
    //Примечание: sizeof(StructWithArray) вернет значение 24 (4int x 4bytes + 1pointer x 8bytes), т.о. первые 24 байта будут заполнены нулями
    //Примечание2: Т.к. 24 - это полный размер memory в байтах, то все его поля будут иметь значения 0
    ...
    //Пример: И т.к. второй байт остался нетронутым, то значение x теперь 255, поскольку все биты второго байта остались единицами, а биты первого были заполнены нулями
